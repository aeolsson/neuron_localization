...
#    true_neuron_positions = np.array([[r*np.cos(theta), r*np.sin(theta), h] for r, theta, h in zip(rs, thetas, hs)])
    ...
    for i, e in enumerate(selected_channels):
        ...
        if i == 0:
            ...
            ax.axhline(y=threshold, xmin=0, xmax=params.fs*duration, color='black', linestyle='--', linewidth=3.0, label='Threshold', zorder=1)
            ...
            ax.axhline(y=threshold, xmin=0, xmax=params.fs*duration, color='black', linestyle='--', linewidth=3.0, zorder=1)
